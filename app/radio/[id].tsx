...
export default function RadioDetailScreen() {
  ...
  const { radios, currentRadio, isPlaying, isLoading, playRadio, togglePlay, toggleFavorite, isFavorite } = useRadioPlayer();
  ...
  useEffect(() => {
    ...
    Animated.timing(stationTransition, { toValue: 1, duration: 380, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start();
  ...
  const stationMotionStyle = { opacity: stationTransition.interpolate({ inputRange: [0, 1], outputRange: [0.72, 1], extrapolate: "clamp" }), transform: [{ translateX: stationTransition.interpolate({ inputRange: [0, 1], outputRange: [stationDirection.current * Math.min(windowWidth * 0.9, 360), 0], extrapolate: "clamp" }) }, { scale: stationTransition.interpolate({ inputRange: [0, 1], outputRange: [0.94, 1], extrapolate: "clamp" }) }] };
