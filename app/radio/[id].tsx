...
export default function RadioDetailScreen() {
  ...
  return <ScreenContainer containerClassName="bg-[#090B12]" className="px-5 pt-3"><Animated.View {...panResponder.panHandlers} style={{ flex: 1, opacity: dismissY.interpolate({ inputRange: [0, 160], outputRange: [1, 0.72], extrapolate: "clamp" }), transform: [{ translateY: dismissY }] }}>{radio.favicon ? <Image source={{ uri: radio.favicon }} style={styles.dynamicBackground} contentFit="cover" cachePolicy="disk" /> : <LinearGradient colors={[`${radio.accent}55`, "#090B12"]} style={styles.dynamicBackground} />}{radio.favicon && <BlurView intensity={78} tint="dark" experimentalBlurMethod="dimezisBlurView" style={styles.dynamicBackground} />}<LinearGradient colors={["#090B12B8", "#090B12F5"]} style={styles.backgroundOverlay} pointerEvents="none" /><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}><View style={styles.header}><Pressable onPress={() => router.back()} style={styles.back}><IconSymbol name="chevron.left" size={22} color="#F5F3EE" /></Pressable><Pressable onPress={() => toggleFavorite(radio.id)} style={styles.back}><IconSymbol name={isFavorite(radio.id) ? "heart.fill" : "heart"} size={20} color={isFavorite(radio.id) ? "#FF6B5F" : "#F5F3EE"} /></Pressable></View><Animated.View style={{ transform: [{ scale: Animated.add(0.52, Animated.multiply(entryProgress, 0.48)).interpolate({ inputRange: [0.52, 1], outputRange: [0.52, 1], extrapolate: "clamp" }) }, { translateY: Animated.add(210, Animated.multiply(entryProgress, -210)) }, { scale: dismissY.interpolate({ inputRange: [0, 420], outputRange: [1, 0.52], extrapolate: "clamp" }) }, { translateY: dismissY.interpolate({ inputRange: [0, 420], outputRange: [0, 210], extrapolate: "clamp" }) }] }}><View style={[styles.artwork, { backgroundColor: `${radio.accent}${lightMode ? "18" : "32"}` }, lightMode && styles.artworkLight]}><View style={[styles.artGlow, { backgroundColor: `${radio.accent}${lightMode ? "35" : "55"}` }]} /><StationLogo radio={radio} size={156} radius={42} /></View></Animated.View><AudioEqualizer playing={active} color={lightMode ? "#C2413E" : radio.accent} /><Text style={styles.eyebrow}>ESTÁS ESCUCHANDO</Text><Text style={styles.name}>{radio.name}</Text><Text style={styles.meta}>{radio.frequency}  ·  {radio.city}  ·  {radio.genre}</Text><View style={styles.actions}><Pressable onPress={() => playRadio(radio)} style={({ pressed }) => [styles.playButton, pressed && { transform: [{ scale: 0.97 }] }]}><IconSymbol name={active ? "pause.fill" : "play.fill"} size={24} color="#090B12" /><Text style={styles.playText}>{isLoading ? "Conectando" : active ? "Pausar" : "Reproducir"}</Text></Pressable><Pressable onPress={openOfficialSite} disabled={!radio.homepage} style={({ pressed }) => [styles.siteButton, !radio.homepage && styles.disabled, pressed && { opacity: 0.75 }]}><IconSymbol name="globe" size={20} color="#F5F3EE" /><Text style={styles.siteText}>Web oficial</Text></Pressable></View><View style={styles.infoCard}><Text style={styles.infoLabel}>SOBRE LA EMISORA</Text><Text style={styles.description}>{radio.description}</Text>{radio.homepage ? <Text style={styles.url} numberOfLines={1}>{radio.homepage.replace(/^https?:\/\//, "").replace(/\/$/, "")}</Text> : <Text style={styles.unavailable}>Página oficial no disponible</Text>}</View></ScrollView></Animated.View></ScreenContainer>;
}
const styles = StyleSheet.create({ dynamicBackground: { ...StyleSheet.absoluteFillObject, opacity: 0.62 }, backgroundOverlay: { ...StyleSheet.absoluteFillObject }, content: { paddingBottom: 35 }, header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 25 }, back: { width: 40, height: 40, borderRadius: 14, backgroundColor: "#FFFFFF0D", borderWidth: 1, borderColor: "#FFFFFF12", alignItems: "center", justifyContent: "center" }, artworkLight: { borderColor: "#D6DCEC" }, artwork: { height: 270, borderRadius: 34, alignItems: "center", justifyContent: "center", overflow: "hidden", marginBottom: 28, borderWidth: 1, borderColor: "#FFFFFF18" }, artGlow: { position: "absolute", width: 230, height: 230, borderRadius: 130, top: 20, opacity: 0.55 }, equalizer: { marginBottom: 12 }, eyebrow: { color: "#A8B0C2", fontSize: 11, fontWeight: "700", letterSpacing: 1.5, marginBottom: 10 }, name: { color: "#F5F3EE", fontSize: 31, fontWeight: "700", letterSpacing: -0.8 }, meta: { color: "#A8B0C2", fontSize: 14, marginTop: 8 }, actions: { flexDirection: "row", gap: 10, marginTop: 26, marginBottom: 30 }, playButton: { flex: 1, minHeight: 52, borderRadius: 18, backgroundColor: "#F5F3EE", flexDirection: "row", gap: 10, alignItems: "center", justifyContent: "center" }, playText: { color: "#090B12", fontSize: 14, fontWeight: "700" }, siteButton: { flex: 1, minHeight: 52, borderRadius: 18, backgroundColor: "#FFFFFF0D", borderWidth: 1, borderColor: "#FFFFFF18", flexDirection: "row", gap: 9, alignItems: "center", justifyContent: "center" }, siteText: { color: "#F5F3EE", fontSize: 14, fontWeight: "600" }, disabled: { opacity: 0.45 }, infoCard: { borderRadius: 22, backgroundColor: "#FFFFFF08", borderWidth: 1, borderColor: "#FFFFFF12", padding: 20 }, infoLabel: { color: "#A8B0C2", fontSize: 10, fontWeight: "700", letterSpacing: 1.4, marginBottom: 12 }, description: { color: "#E5E4E1", fontSize: 15, lineHeight: 23 }, url: { color: "#FF8C7F", fontSize: 12, marginTop: 15 }, unavailable: { color: "#777F91", fontSize: 12, marginTop: 15 }, notFound: { paddingTop: 100, alignItems: "center" }, notFoundTitle: { color: "#F5F3EE", fontSize: 20, fontWeight: "700" }, notFoundText: { color: "#8D95A7", fontSize: 13, textAlign: "center", marginTop: 10 } });
