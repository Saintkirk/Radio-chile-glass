...
export default function ExploreScreen() {
  ...
  return <ScreenContainer containerClassName="bg-background" className="px-5 pt-3">
    <SectionList
      ...
      renderItem={(info) => { const item = info.item; const playing = isPlaying && currentRadio?.id === item.id; return <ItunesRadioCard radio={item} lightMode={lightMode} onOpen={() => { router.push(`/radio/${item.id}`); }} onPlay={() => playing ? togglePlay() : playRadio(item)} playing={playing} loading={isLoading && currentRadio?.id === item.id} />; }}
