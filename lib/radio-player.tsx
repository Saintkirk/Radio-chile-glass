...
import { RADIOS, type Radio } from "./radios";
...
export function RadioPlayerProvider({ children }: { children: ReactNode }) {
  ...
  const playRadio = async (radio: Radio) => {
    ...
    try {
      ...
      playerRef.current?.play(); setIsPlaying(true);
  ...
  const toggleFavorite = (radioId: string) => {
    setFavorites((previous) => {
      const next = previous.includes(radioId) ? previous.filter((id) => id !== radioId) : [...previous, radioId];
      AsyncStorage.setItem("radio-favorites", JSON.stringify(next)).catch(() => undefined);
      return next;
    });
  };

  const value = useMemo(() => ({ currentRadio, isPlaying, isLoading, favorites, playRadio, togglePlay, toggleFavorite, isFavorite: (id: string) => favorites.includes(id) }), [currentRadio, isPlaying, isLoading, favorites]);
