...
const MAX_GESTURE_VELOCITY = 2800;
const SWIPE_FLING_MULTIPLIER = 0.28;
...
function useSlotCardAnimatedStyle(
  ...
  return useAnimatedStyle(() => {
    ...
    const rotation = interpolate(clamped, [-4, -2, 0, 2, 4], [42, 24, 0, -24, -42], Extrapolation.CLAMP);
    const scale = interpolate(distance, [0, 1, 2, 3, 5], [1, 0.92, 0.82, 0.72, 0.55], Extrapolation.CLAMP);
    const opacity = interpolate(distance, [0, 1, 2, 3, 5], [1, 0.95, 0.75, 0.4, 0], Extrapolation.CLAMP);
...
const SlotCard = memo(function SlotCard({
  ...
  const centerGlow = useAnimatedStyle(() => {
    if (!isCenter) return { opacity: 0 };
    return {
      opacity: interpolate(spinProgress.get(), [0, 1], [0.12, 0.04], Extrapolation.CLAMP),
    };
  ...
  return (
    <Animated.View
      renderToHardwareTextureAndroid={isCenter}
      shouldRasterizeIOS={isCenter}
...
export function CoverFlowCarousel({
  ...
  const selectSlot = useCallback((slot: number) => {
    ...
    wheelOffset.set(withTiming(-boundedSlot, {
      duration: 280 + Math.abs(boundedSlot) * 55,
  ...
  const spin = useCallback(() => {
    ...
    spinProgress.set(withSequence(
      withTiming(0.12, { duration: 280, easing: Easing.out(Easing.cubic) }),
      withTiming(0.82, { duration: 1800, easing: Easing.linear }),
      withTiming(1, { duration: 600, easing: Easing.out(Easing.cubic) }),
    ));
    wheelOffset.set(withSequence(
      withTiming(-targetSlots * 0.12, { duration: 280, easing: Easing.in(Easing.cubic) }),
      withTiming(-targetSlots * 0.82, { duration: 1800, easing: Easing.linear }),
      withTiming(-targetSlots, { duration: 600, easing: Easing.out(Easing.cubic) }, (finished) => {
  ...
  useEffect(() => {
    ...
    if (circularDiff > 0 && circularDiff < 2) return;
  ...
  const panGesture = useMemo(() => Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      if (Math.abs(translation) > DRAG_LIMIT) {
        const excess = Math.abs(translation) - DRAG_LIMIT;
        const sign = translation > 0 ? 1 : -1;
        translation = sign * (DRAG_LIMIT + excess * 0.22);
      }
    ...
    .onEnd((event) => {
      ...
      const projectedTranslation = event.translationX + limitedVelocity * SWIPE_FLING_MULTIPLIER;
      let slots = 0;
      if (Math.abs(projectedTranslation) >= SWIPE_DISTANCE || Math.abs(limitedVelocity) >= SWIPE_VELOCITY) {
        const direction = projectedTranslation < 0 ? 1 : -1;
        const velocitySlots = Math.min(3, Math.floor(Math.abs(limitedVelocity) / 900));
        const distanceSlots = Math.min(2, Math.floor(Math.abs(projectedTranslation) / (CARD_STEP * 0.65)));
        slots = direction * Math.max(1, velocitySlots, distanceSlots);
      }
      if (slots === 0 || reduceMotion) {
        if (reduceMotion && slots !== 0) runOnJS(commitSlot)(slots);
        else wheelOffset.set(withTiming(0, { duration: 220, easing: Easing.bezier(0.25, 0.8, 0.3, 1) }));
        return;
      }
      const duration = Math.min(420, 260 + Math.abs(slots) * 55);
      wheelOffset.set(withTiming(-slots, {
        duration,
        easing: Easing.bezier(0.22, 0.9, 0.28, 1),
      }, (finished) => {
        if (finished && isMounted.get()) runOnJS(commitSlot)(slots);
      }));
